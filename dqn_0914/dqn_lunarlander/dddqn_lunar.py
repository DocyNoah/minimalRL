...
class Qnet(nn.Module):
    ...
    def forward(self, x):
        ...
        val_out = self.val_net(base_out)
        adv_out = self.adv_net(base_out)

        q_out = val_out + (adv_out - adv_out.mean())
...
def train(q, q_target, memory, optimizer):
    ...
    for i in range(64):
        ...
        next_out = q(s_prime)
        next_terget_out = q_target(s_prime)

        a_prime = next_out.max(1)[1].unsqueeze(-1)
        max_q_prime = next_terget_out.gather(1, a_prime)
...
def main():
    ...
    wandb.run.name = "dddqn_lunar_{}".format(current_time())
    print(current_time())
    main_time = time.time()
    ...
    for n_epi in range(5000):
        ...
        epsilon = max(0.01, 0.80 - 0.01 * (n_epi / 50))
        ...
        if n_epi % print_interval == 0 and n_epi != 0:
            ...
            if best_mean_reward < mean_reward:
                torch.save(q.state_dict(), "DDDQN-best.dat")
    ...
    reward_path = os.path.join(".", "output_data", "dddqn_reward_{}".format(main_time))
    loss_path = os.path.join(".", "output_data", "dddqn_loss_{}".format(main_time))
    ...
    # # Test render
    # q.load_state_dict(torch.load("DDDQN-best.dat"))
    # fps = 60
    #
    # for _ in range(10):
    #     s = env.reset()
