...
class Network(nn.Module):
    def __init__(self, obs_size, n_actions, hidden_size: int = 128):
...
class DQN(nn.Module):
    def __init__(self):
        ...
        self.net = Network(4, 2, 128)
        self.target_net = Network(4, 2, 128)
...
def main():
    ...
    for n_epi in range(5000):  # TODO epoch -> hypermarameter
        # TODO epsilon decay || linear -> common
        ...
        while not done:
            ...
            a = dqn.sample_action(s_, epsilon)  # a = argmax Q(a|s)  # TODO DeterActor get_action() 만들기
        ...
        # Train per episode
        if memory.size() > 2000:
            total_loss += dqn.train_step(memory)
            train_steps += 1
...
# TODO dqn, network, agent, hypermarameter 분리
# TODO device 추가
